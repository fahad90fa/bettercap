...
struct MacAddress {
    ...
    // The U/L bit (2nd-lowest bit of the first octet): 1 means this MAC was
    // randomly generated by the device (iOS Private Address, Android
    // per-network MAC randomization, etc.) rather than assigned from a
    // vendor's real OUI block. Such devices can rotate to a new MAC/IP
    // pair over time, which is why they can look like "wrong" or
    // duplicate entries in a device table if not flagged explicitly.
...
struct IPv4Address {
    ...
    // addr packs octets LSB-first (see FromString), so comparing it
    // directly does NOT sort in the usual left-to-right dotted-decimal
    // order once addresses span different high octets. This repacks into
    // normal big-endian order purely for human-readable sorting/display.
    uint32_t SortKey() const {
        return (((addr >> 0) & 0xFF) << 24) | (((addr >> 8) & 0xFF) << 16) |
